...
#         if user_id not in self.window:
#             return

#         while (
#             self.window[user_id]
#             and self.window[user_id][0] <= current_time - self.window_size
#         ):
#             self.window[user_id].popleft()

#         if not self.window[user_id]:
#             del self.window[user_id]

#     def can_send_message(self, user_id: str) -> bool:
#         current_time = time.time()
#         self._cleanup_window(user_id, current_time)

#         return len(self.window.get(user_id, [])) < self.max_requests
...
#         if user_id not in self.window:
#             self.window[user_id] = deque()

#         self.window[user_id].append(time.time())
#         return True
...
#         return self.window[user_id][0] + self.window_size - current_time


# # Демонстрація роботи
# def test_rate_limiter():
#     # Створює rate limiter: вікно 10 секунд, 1 повідомлення
#     limiter = SlidingWindowRateLimiter(window_size=10, max_requests=1)

#     # Симулює потік повідомлень від користувачів (послідовні ID від 1 до 20)
#     print("\n=== Симуляція потоку повідомлень ===")
#     for message_id in range(1, 11):
#         # Симулює різних користувачів (ID від 1 до 5)
